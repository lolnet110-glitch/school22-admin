...
function buildDemoPayload() {
  if (state.demoPayload) return state.demoPayload;
  ...
  const ratings = snapshot.ratings.map(row => ({
    ...row,
    categories: row.categories.map(categoryScore => {
      const category = {...categoryById.get(Number(categoryScore.id))};
      const subcategories = categoryScore.subcategories.map(score => {
        const subcategory = {...subcategoryById.get(Number(score.id))};
        const uniform = subcategory.code === "КР-08.01" ? snapshot.uniform_by_class[row.class_id] : null;
        return {...subcategory, points: score.points, raw_points: score.points, events: [], ...(uniform ? {uniform_summary: uniform} : {})};
      });
      return {...category, points: categoryScore.points, raw_points: categoryScore.points, subcategories,
        ...(subcategories.some(item => item.code === "КР-08.01") ? {uniform_summary: snapshot.uniform_by_class[row.class_id]} : {})};
    })
  }));
  state.demoPayload = {classes: snapshot.classes, categories, ratings, uniformByClass: snapshot.uniform_by_class};
...
function demoApi(path, options = {}) {
  ...
  if (method !== "GET") {
    toast("Демо-режим: реальные данные не изменяются");
    return {id: Date.now(), message: "Демонстрационное действие"};
  }
  if (path === "/api/classes") return JSON.parse(JSON.stringify(demo.classes));
  if (path === "/api/categories") return JSON.parse(JSON.stringify(demo.categories));
  if (path === "/api/ratings/classes") return JSON.parse(JSON.stringify(demo.ratings));
  const classMatch = path.match(/^\/api\/classes\/(\d+)\/(uniform-checks|category-scores|subcategory-events)$/);
  if (classMatch) {
    const classId = Number(classMatch[1]);
    const section = classMatch[2];
    if (section === "uniform-checks") return JSON.parse(JSON.stringify(demo.uniformByClass[classId]));
    if (section === "category-scores") return JSON.parse(JSON.stringify(demo.ratings.find(row => Number(row.class_id) === classId)?.categories || []));
    return [];
  }
  return {};
}
...
async function api(path, options = {}) {
  if (state.demoMode) return demoApi(path, options);
